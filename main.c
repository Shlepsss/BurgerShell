...
static void handle_key(ShellState* st, char key) {
    ...
    //if(!mouse_present){
        if(key == 87){set_mouse_position(get_mouse_x(),get_mouse_y() - 2);}
        if(key == 65){set_mouse_position(get_mouse_x() -2 ,get_mouse_y());}
        if(key == 83){set_mouse_position(get_mouse_x(),get_mouse_y() + 2);}
        if(key == 68){set_mouse_position(get_mouse_x() + 2,get_mouse_y());}
